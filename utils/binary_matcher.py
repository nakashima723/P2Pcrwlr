...
class PeerBinaryMatcher:
    ...
    def binary_match(self):
        ...
        if dl_target is None or not os.path.exists(
            os.path.join(self.folder, dl_target)
        ):
        ...
        all_folders = [
            ...
            if os.path.isdir(os.path.join(self.folder, item)) and "_" in item
        ]
        ...
        for folder in all_folders:
            ...
            log_files = [
                f for f in os.listdir(current_peer_folder) if f.endswith(".log")
            ]

            if not log_files:
                # ログファイルが存在しない場合、新しいログファイルを作成
                error_log_path = os.path.join(
                    current_peer_folder, f"エラー_ログファイルが存在しません_{folder}.log"
                )
            ...
            bin_files = [
                f
                for f in os.listdir(current_peer_folder)
                if f.endswith(".bin") and not f.startswith(("V_", "mis_"))
            ]
