...
def merge_ip_ranges(ip_ranges):
    ...
    for ip_range in ip_ranges[1:]:
        if current_range.overlaps(ip_range) or current_range.broadcast_address + 1 == ip_range.network_address:
            # collapse_addressesの結果をリストに変換し、最初の要素を取得する
            current_range = list(ipaddress.collapse_addresses([current_range, ip_range]))[0]
        else:
...
def process_data(data):
    ...
    for line in data:
        ...
        if country == 'JP':
            if ip_type == 'ipv4':
                # CIDR表記を計算する
                ip_net = ipaddress.ip_network(f'{ip_start}/{32-int(math.log2(ip_size))}', strict=False)
                ipv4_output.append(str(ip_net))
            elif ip_type == 'ipv6':
                # CIDR表記を計算する
                ip_net = ipaddress.ip_network(f'{ip_start}/{128-int(math.log2(ip_size))}', strict=False)
...
def update_data_and_settings():
    ...
    try:
        with urllib.request.urlopen(url) as response:
            ...
            ipv4_output, ipv6_output = merge_ip_ranges(ipv4_output), merge_ip_ranges(ipv6_output)
